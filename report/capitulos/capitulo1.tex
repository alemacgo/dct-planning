...
fragmentos de lógica de segundo orden y permite establecer garantías sobre la
...
\item $I$ es el \textbf{estado inicial}, con $I\subseteq C$. Los estados son
...
\item $F$ es el \textbf{estado final o meta}, con $F \subseteq C$.
\end{itemize}
\end{definition}
...
\begin{Verbatim}[commandchars=\\\{\},
codes={\catcode`$=3\catcode`^=7}]
estado: sobreMesa(bloque0) $\land$ {\color{red} sobreMesa(bloque1)} $\land$ 
        libre(bloque0) $\land$ libre(bloque1) $\land$ {\color{red} manoVacía}
\textit{acción: recoger_bloque(bloque1)}
estado: {\color{blue} enMano(bloque1)} $\land$ sobreMesa(bloque0) $\land$ 
        {\color{red} libre(bloque0)} $\land$ libre(bloque1)
\textit{acción: colocar_sobre(bloque1, bloque0)}
estado: {\color{blue} sobre(bloque1, bloque0)} $\land$ sobreMesa(bloque0) $\land$ 
        {\color{blue}manoVacía} $\land$ libre(bloque1)
\end{Verbatim}
