...
lograr un objetivo concreto \cite{russell:book}. En este trabajo se manejará
...
formal.
\begin{definition}
Una \textbf{interpretación} $i$ sobre $\A$ es una función que asigna
elementos del conjunto de variables, $\text{VAR}$,
a elementos de $|\A|$; o asigna a relaciones $R^k$, donde $R \not\in \sigma$, a
tuplas en $|\A|^k$.

Como ejemplo, dos posibles interpretaciones de la fórmula 
$(\exists xy) (T(x, y))$ son $i(x) = 1$ e $i(T) = \tup{0,2}$.
\end{definition}
...
dominio incluye todas las variables y relaciones libres.
Se definen inductivamente las condiciones necesarias y suficientes para que 
una fórmula $\Phi \in \LSO(\sigma)$ sea cierta en \A\ bajo la
interpretación $i$:
\[
\begin{array}{lcl}
