...
(define (domain qbf)
	...
	(:predicates
		...
		(holds_and_7 ?x0)
		(holds_and_9 ?x0 ?x1)
		(holds_exists_23 ?x0 ?x1)
		(holds_forall_24 ?x0 ?x1)
		(holds_or_20 ?x0)
		(holds_or_22 ?x0 ?x1 ?x2)
		(holds_or_8 ?x0)
		(holds_so-exists_r )
		(holds_so-forall_t )
		(iterate_t )
	...
	(:action establish_soexist_r
		:precondition	(and (holds_forall_24 ?y max))
		:effect	(and (not (proof)) (not (holds_forall_24 ?y max)) (holds_so-exists_r)) 
	)
	(:action establish_forall_24_base
		:parameters	(?y)
		:precondition	 (and (holds_exists_23 zero ?y) (proof))
		:effect		(and (holds_forall_24 ?y zero)  (not (holds_exists_23 zero ?y)))
	)
	(:action establish_forall_24_inductive
		:parameters	(?y ?iv0 ?iv1)
		:precondition	(and (holds_forall_24 ?y ?iv0) (suc ?iv0 ?iv1) (holds_exists_23 ?iv1 ?y) (proof))
		:effect		(and  (not (holds_forall_24 ?y ?iv0))  (not (holds_exists_23 ?iv1 ?y)) (holds_forall_24 ?y ?iv1))
	)
	(:action establish_exists_23
		:parameters	(?c ?y ?x)
		:precondition	 (and (holds_or_22 ?c ?y ?x) (proof))
		:effect		(and (holds_exists_23 ?c ?y) (not (holds_or_22 ?c ?y ?x))))
	(:action establish_or_22_0
		:parameters	(?c ?y ?x)
		:precondition	 (and (holds_and_9 ?c ?x) (proof))
		:effect		(and (holds_or_22 ?c ?y ?x) (not (holds_and_9 ?c ?x)))
	)
	(:action establish_or_22_1
		:parameters	(?c ?y ?x)
		:precondition	 (and (holds_and_21 ?y ?x) (proof))
		:effect		(and (holds_or_22 ?c ?y ?x) (not (holds_and_21 ?y ?x)))
	)
	...
	(:action establish_and_21
		:parameters	(?y ?x)
		:precondition	(and (n ?x ?y) (holds_or_20 ?x) (proof))
		:effect		(and (holds_and_21 ?y ?x)  (not (holds_or_20 ?x)))
	)
