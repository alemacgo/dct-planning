...
(define (domain qbf)
	...
	(:action establish_soexist_r
		...
		:effect	(and  (not (proof_operator_level)) (not (holds_forall_24  max)) (holds_so-exists_r) (not (proof_atom_level))
					(forall (?z)
						(when (holds_forall_24 ?z)
							 (not (holds_forall_24 ?z)))
					)
					(forall (?z)
						(when (holds_exists_23 ?z)
							 (not (holds_exists_23 ?z)))
					)
					(forall (?z ?y)
						(when (holds_or_22 ?z ?y)
							 (not (holds_or_22 ?z ?y)))
					)
					(forall (?z ?y)
						(when (holds_and_9 ?z ?y)
							 (not (holds_and_9 ?z ?y)))
					)
					(forall (?z ?y)
						(when (holds_and_21 ?z ?y)
							 (not (holds_and_21 ?z ?y)))
					)
					(forall (?z)
						(when (holds_or_8 ?z)
							 (not (holds_or_8 ?z)))
